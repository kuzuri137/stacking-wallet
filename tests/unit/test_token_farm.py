from scripts.deploy import deploy_token_farm_and_avocado_token, KEPT_BALANCE
...
def test_add_allowed_tokens():
    ...
    token_farm, avocado_token = deploy_token_farm_and_avocado_token()
    # Act
    token_farm.addAllowedTokens(avocado_token.address, {"from": account})
    # Assert
    assert token_farm.allowedTokens(0) == avocado_token.address
    with pytest.raises(exceptions.VirtualMachineError):
        token_farm.addAllowedTokens(avocado_token.address, {"from": non_owner})
...
def test_set_price_feed_contract():
    ...
    token_farm.setPriceFeedContract(
        avocado_token.address, get_contract("eth_usd_price_feed"), {"from": account}
    )
    # Assert
    assert token_farm.tokenPriceFeedMapping(avocado_token.address) == get_contract(
        "eth_usd_price_feed"
    )
    with pytest.raises(exceptions.VirtualMachineError):
        token_farm.setPriceFeedContract(
...
def test_stake_tokens(amount_staked):
    ...
    token_farm, avocado_token = deploy_token_farm_and_avocado_token()
    # Act
    avocado_token.approve(token_farm.address, amount_staked, {"from": account})
    token_farm.stakeTokens(amount_staked, avocado_token.address, {"from": account})
    # Assert
    assert (
        token_farm.stakingBalance(avocado_token.address, account.address)
        == amount_staked
    )
...
def test_unstake_tokens(amount_staked):
    ...
    if network.show_active() not in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        pytest.skip("Only for local testing")
    account = get_account()
    token_farm, avocado_token = test_stake_tokens(amount_staked)
    # Act
    token_farm.unstakeTokens(avocado_token.address, {"from": account})
    # Assert
    assert avocado_token.balanceOf(account.address) == KEPT_BALANCE
    assert token_farm.stakingBalance(avocado_token.address, account.address) == 0
...
def test_get_token_eth_price():
    ...
    token_farm, avocado_token = deploy_token_farm_and_avocado_token()
    # Act / Assert
    assert token_farm.getTokenEthPrice(avocado_token.address) == (
        INITIAL_PRICE_FEED_VALUE,
        DECIMALS,
    )
...
def test_get_user_token_staking_balance_eth_value(amount_staked):
    ...
    eth_balance_token = token_farm.getUserTokenStakingBalanceEthValue(
        account.address, avocado_token.address
    )
...
def test_issue_tokens(amount_staked):
    ...
    account = get_account()
    token_farm, avocado_token = test_stake_tokens(amount_staked)
    starting_balance = avocado_token.balanceOf(account.address)
    # Act
    token_farm.issueTokens({"from": account})
    # Assert
    assert (
        avocado_token.balanceOf(account.address)
